...
你可以粘贴文本、选择或拖入最多 20 个原生支持文件，或打开“使用示例”菜单。v0.14.0 原生支持文本、Markdown、DOCX/PPTX/XLSX、表格、HTML、JSON/XML、Notebook、EPUB、字幕和安全代码文本；PDF 当前只提供 Core fallback，不做内置 OCR。需要时可打开“支持能力”，为当前窗口明确选择已经安装并检测到的 Docling / MarkItDown，或选择本机 Pandoc 可执行文件；选择不保存，也不会自动安装。导入不会自动调用 Provider。粘贴材料仍走原有的一次生成路径；已导入文档才使用有界的本地分析、切块和计划。

第一次使用建议选择内置示例；示例会填入材料并推荐卡片模式，但不会自动生成。

## 4. 选择模式与生成

“卡片模式”始终可见；卡片数量、答案长度和输出语言位于默认收起的生成设置中。公开模式适合概念、定义、考试、快速回顾、对比、流程、公式或易错点。v0.14.0 当前不开放 Cloze 选择；内部兼容检查只负责 fail closed，不会创建或修改笔记类型与字段。已导入文档可选择 Fast、Standard 或 Deep：界面显示当前计划的具体调用数、12 次硬上限和最多 5 张 balanced 候选卡；历史范围不是每次运行的承诺。
